...
Puppet::Type.type(:proxy_mysql_galera_hostgroup).provide(:proxysql, parent: Puppet::Provider::Proxysql) do
  ...
  def self.instances
    ...
    hostgroups = mysql([defaults_file, '-NBe',
                        'SELECT `writer_hostgroup`, `backup_writer_hostgroup`, `reader_hostgroup`, `offline_hostgroup`, `active`, `max_writers`, `writer_is_also_reader`, `comment` FROM `mysql_galera_hostgroups`'].compact).split(%r{\n})

    # To reduce the number of calls to MySQL we collect all the properties in
    # one big swoop.
    hostgroups.each do |line|
      writer_hostgroup, backup_writer_hostgroup, reader_hostgroup, offline_hostgroup, active, max_writers, writer_is_also_reader, comment = line.split(%r{\t})
  ...
  def create
    ...
    query = 'INSERT INTO `mysql_galera_hostgroups` (`writer_hostgroup`, `backup_writer_hostgroup`, `reader_hostgroup`, `offline_hostgroup`, `active`, `max_writers`, `writer_is_also_reader`, `comment`)'
    query << " VALUES (#{writer_hostgroup}, #{backup_writer_hostgroup}, #{reader_hostgroup}, #{offline_hostgroup}, #{active}, #{max_writers}, #{writer_is_also_reader}, '#{comment}')"
  ...
  def active=(value)
    ...
    reader_hostgroup = @resource.value(:reader_hostgroup)
    offline_hostgroup = @resource.value(:offline_hostgroup)
    query = "UPDATE mysql_galera_hostgroups SET `active` = #{value}"
    query << " WHERE `writer_hostgroup` =  #{writer_hostgroup} AND `backup_writer_hostgroup` = #{backup_writer_hostgroup} AND `reader_hostgroup` = #{reader_hostgroup} AND `offline_hostgroup` = #{offline_hostgroup}"
    mysql([defaults_file, '-e', query].compact)
  ...
  def max_writers=(value)
    ...
    reader_hostgroup = @resource.value(:reader_hostgroup)
    offline_hostgroup = @resource.value(:offline_hostgroup)
    query = "UPDATE mysql_galera_hostgroups SET `max_writers=` = #{value}"
    query << " WHERE `writer_hostgroup` =  #{writer_hostgroup} AND `backup_writer_hostgroup` = #{backup_writer_hostgroup} AND `reader_hostgroup` = #{reader_hostgroup} AND `offline_hostgroup` = #{offline_hostgroup}"
    mysql([defaults_file, '-e', query].compact)
  ...
  def writer_is_also_reader=(value)
    ...
    reader_hostgroup = @resource.value(:reader_hostgroup)
    offline_hostgroup = @resource.value(:offline_hostgroup)
    query = "UPDATE mysql_galera_hostgroups SET `writer_is_also_reader` = #{value}"
    query << " WHERE `writer_hostgroup` =  #{writer_hostgroup} AND `backup_writer_hostgroup` = #{backup_writer_hostgroup} AND `reader_hostgroup` = #{reader_hostgroup} AND `offline_hostgroup` = #{offline_hostgroup}"
    mysql([defaults_file, '-e', query].compact)
