  ...
  context 'extended testing' do
    ...
    it 'works idempotently with no errors' do
      ...
      proxy_mysql_galera_hostgroup { '5-2-10-11':
        ...
        backup_writer_hostgroup => 2,
        offline_hostgroup       => 11,
        comment                 => 'Test MySQL Galera Cluster 5-2-10-11',
      }

      proxy_mysql_galera_hostgroup { '3-20-6-30':
        ensure                  => 'absent',
        reader_hostgroup        => 6,
        writer_hostgroup        => 3,
        backup_writer_hostgroup => 20,
        offline_hostgroup       => 30,
        comment                 => 'Test MySQL Galera Cluster 3-20-6-30',
      }
    ...
    describe command("mysql -NB -e 'SELECT comment FROM mysql_galera_hostgroups WHERE writer_hostgroup = 5 AND backup_writer_hostgroup = 2 AND reader_hostgroup = 10 AND offline_hostgroup = 11;'") do
      its(:exit_status) { is_expected.to eq 0 }
      its(:stdout) { is_expected.to match '^Test MySQL Galera Cluster 5-2-10-11$' }
    end

    describe command("mysql -NB -e 'SELECT comment FROM mysql_galera_hostgroups WHERE writer_hostgroup = 6 AND backup_writer_hostgroup = 3 AND reader_hostgroup = 20 AND offline_hostgroup = 30;'") do
      its(:exit_status) { is_expected.to eq 0 }
      its(:stdout) { is_expected.to eq('') }
    end
