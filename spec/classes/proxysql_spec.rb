    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context 'proxysql class without any parameters' do
          ...
          it { is_expected.to contain_class('proxysql::repo').that_comes_before('Class[proxysql::config]') }
          it { is_expected.to contain_class('proxysql::config').that_comes_before('Class[proxysql::install]') }
          it { is_expected.to contain_class('proxysql::install').that_comes_before('Class[proxysql::service]') }
          ...
          it { is_expected.to contain_class('proxysql::reload_config').that_comes_before('Class[proxysql::cluster]') }
          it { is_expected.to contain_class('proxysql::cluster').that_comes_before('Class[proxysql::configure]') }
          ...
          it do
            is_expected.to contain_file('proxysql-main-config-file').with(ensure: 'file',
                                                                          owner: sys_user,
                                                                          group: sys_group,
                                                                          mode: '0640',
